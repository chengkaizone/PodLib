...
/** 进入刷新状态的回调 */
public typealias MJRefreshComponentRefreshingBlock = ()->Void
...
public class MJRefreshComponent: UIView {
    ...
    var scrollViewOriginalInset = UIEdgeInsets.init(top: 0, left: 0, bottom: 0, right: 0) // 记录scrollView刚开始的inset
    
    weak var scrollView: UIScrollView? // 父控件
    
    public var refreshingBlock: MJRefreshComponentRefreshingBlock? // 正在刷新的回调
    
    public var beginRefreshingCompletionBlock: MJRefreshComponentbeginRefreshingCompletionBlock? // 开始刷新后的回调(进入刷新状态后的回调)
    
    public var endRefreshingCompletionBlock: MJRefreshComponentEndRefreshingCompletionBlock? // 结束刷新的回调
    ...
    public var autoChangeAlpha = false {
        didSet{
            if self.isRefreshing() {
                return
            }
    ...
    func mj_textWith() -> CGFloat {
        ...
        if let text = self.text {
            if text.count > 0 {
                ...
                stringWidth = (self.text?.boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:self.font], context: nil).size.width)!
            }
        }
        return stringWidth
    }
