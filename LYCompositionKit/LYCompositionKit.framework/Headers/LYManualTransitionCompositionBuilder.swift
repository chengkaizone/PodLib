//
//  LYManualTransitionCompositionBuilder.swift
//  LYCompositionKit
//
 ...
 *
 */
class LYManualTransitionCompositionBuilder: NSObject, LYCompositionBuilder {
    ...
    func buildVideoCompositionAndInstructions(_ renderSize:CGSize) ->AVMutableVideoComposition {
        ...
        for i:Int in 0 ..< self.passThroughTimeRanges.count {
            ...
            // 轨道交替索引
            let alertnatingIndex = i % 2;
            let currentTrack = compositionVideoTracks[alertnatingIndex];
            ...
            if item.asset != nil {// 视频的情况
                let tracks = item.asset.tracks(withMediaType: AVMediaType.video)
                assetTrack = tracks.first
            }
            
            let passTimeRange = self.passThroughTimeRanges[i]
            
            var instruction: AVVideoCompositionInstructionProtocol!
            // 处理定制层指令
            if videoComposition.customVideoCompositorClass != nil {
                instruction = APLCustomVideoCompositionInstruction(sourceTrackIDs: [NSNumber(integerLiteral: Int(compositionVideoTracks[0].trackID)), NSNumber(integerLiteral: Int(compositionVideoTracks[1].trackID))], forTimeRange: passTimeRange, transitionType: nextItem?.videoTransition?.transitionType)
            } else {
                let tmpInstruction: AVMutableVideoCompositionInstruction = AVMutableVideoCompositionInstruction()
                tmpInstruction.timeRange = passTimeRange
                // 配置通过层的视频指令
                let layerInstruction = AVMutableVideoCompositionLayerInstruction(assetTrack: currentTrack);
                layerInstruction.assetTrack = assetTrack;
                layerInstruction.region = item.region
                layerInstruction.configTransform(renderSize, option)
                tmpInstruction.layerInstructions = [layerInstruction]
            ...
            if i < self.transitionTimeRanges.count {
                ...
                let foregroundTrack = compositionVideoTracks[alertnatingIndex];// 前景层
                let backgroundTrack = compositionVideoTracks[1 - alertnatingIndex];
                
                let transitionTimeRange = self.transitionTimeRanges[i];
                ...
                fromLayerInstruction.assetTrack = assetTrack;
                fromLayerInstruction.region = item.region
                fromLayerInstruction.configTransform(renderSize, option)
                
                let toLayerInstruction = AVMutableVideoCompositionLayerInstruction(assetTrack: backgroundTrack);
                // 这里要获取下一个assetTrack
                let toItem = videos[i + 1];
                if toItem.asset != nil {// 转到的视频片段
                    let toAssetTrack = toItem.asset.tracks(withMediaType: AVMediaType.video)[0];
                    toLayerInstruction.assetTrack = toAssetTrack;
                    toLayerInstruction.region = toItem.region
                    toLayerInstruction.configTransform(renderSize, option)
                    instruction.layerInstructions = [fromLayerInstruction, toLayerInstruction];
                } else {
                    instruction.layerInstructions = [fromLayerInstruction, fromLayerInstruction];
                }
                compositionInstructions.append(instruction);
                // 视频原音的音量
                let videoVolume = timeline.videoVolume
                if compositionAudioTracks.count > alertnatingIndex {// 视频中可能不包含音频轨道
                    let parametersA = AVMutableAudioMixInputParameters(track: compositionAudioTracks[alertnatingIndex])
                    if item.muteEnabled == false {
                ...
                if compositionAudioTracks.count > 1 - alertnatingIndex {// 视频中可能不包含音频轨道
                    let parametersB = AVMutableAudioMixInputParameters(track: compositionAudioTracks[1 - alertnatingIndex])
    ...
    /** 根据转场类型创建转场算法 */
    func buildVideoComposition(_ videoComposition:AVMutableVideoComposition) {
        
        let transitionInstructions = self.transitionInstructionsInVideoComposition(videoComposition);
        
        for instructions:LYTransitionInstructions in transitionInstructions {
            let timeRange = instructions.compositionInstruction.timeRange;
            let fromLayer = instructions.fromLayerInstruction;
            let toLayer = instructions.toLayerInstruction;
            
            let renderSize = videoComposition.renderSize;

            if instructions.transition != nil {
                // 配置转场
                instructions.transition.configLayerInstruction(fromLayer!, toLayerInstruction: toLayer!, renderSize: renderSize, timeRange: timeRange);
            }
    ...
    /** 创建转场指令 */
    func transitionInstructionsInVideoComposition(_ vc:AVVideoComposition) ->[LYTransitionInstructions] {
        
        var transitionInstructions = [LYTransitionInstructions]();
        ...
        for vci in compositionInstructions {
            ...
            let instructions = LYTransitionInstructions();
            instructions.compositionInstruction = vci;
            // 转场layer顺序已经计算出来了
            instructions.fromLayerInstruction = vci.layerInstructions[0] as? AVMutableVideoCompositionLayerInstruction
            instructions.toLayerInstruction = vci.layerInstructions[1] as? AVMutableVideoCompositionLayerInstruction
            
            let transition = self.timeline.videos[index].videoTransition
            // 添加指定的转场
