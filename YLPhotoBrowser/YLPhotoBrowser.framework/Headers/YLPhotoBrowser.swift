...
public class YLPhotoBrowser: UIViewController {
    ...
    public var getTransitionImageView: GetTransitionImageView? {
        didSet {
            if singlePhoto != nil {
                editTransitioningDelegate(singlePhoto,isBack: false)
            } else {
    ...
    // 初始化
    public convenience init(_ index: Int, _ delegate: YLPhotoBrowserDelegate? = nil) {
        self.init()
    ...
    // 直接传递一个图片数据进来
    public convenience init(_ photo: YLPhoto) {
        self.init()
    ...
    override public func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if keyPath == "view.frame" {
            ...
            DispatchQueue.main.async { [weak self] in
                self?.collectionView.scrollToItem(at: IndexPath.init(row: self?.currentIndex ?? 0, section: 0), at: UICollectionView.ScrollPosition.left, animated: false)
            }
        }
    }
    ...
    override public func viewDidLoad() {
        ...
        collectionView.scrollToItem(at: IndexPath.init(row: currentIndex, section: 0), at: UICollectionView.ScrollPosition.left, animated: false)
    ...
    private func layoutUI() {
        ...
        if let count = delegate?.epPhotoBrowserGetPhotoCount(), count > 1 {
