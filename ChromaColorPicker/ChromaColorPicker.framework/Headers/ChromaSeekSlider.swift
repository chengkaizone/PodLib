//
//  ChromaSeekSlider.swift
//  LineCal
//
//  Created by tony on 2022/12/3.
//  Copyright © 2022 chengkaizone. All rights reserved.
//

import UIKit

public protocol ChromaSeekSliderDelegate: NSObjectProtocol {
    
    /// 滑动进度
    func seekSlider(slider: ChromaSeekSlider, progress: CGFloat)
    
}

public class ChromaSeekSlider: UIControl, ChromaControlStylable {
    
    public weak var sliderDelegate: ChromaSeekSliderDelegate?
    ...
    private func moveHandle(to value: CGFloat) {
        ...
        let x = xPos - (size.width / 2)
        handle.frame = CGRect(origin: CGPoint(x: x, y: 0), size: size)
        
        // 计算进度
        
        let progressWidth = bounds.width - horizontalPadding * 2
        let progressPos = progressWidth - (xPos - horizontalPadding)
        if sliderDelegate != nil {
            sliderDelegate?.seekSlider(slider: self, progress: progressPos / progressWidth)
        }
    }
}
